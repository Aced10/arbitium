from itertools import permutations

from app.core.exchanges import fetch_prices, EXCHANGES
from app.core.config import SYMBOLS, MIN_PROFIT
from app.services.persistence_service import save_price_snapshot, save_opportunities
# from app.ml.predictor import predict_opportunity

async def calculate_opportunities(min_profit: float = MIN_PROFIT):
    """
    Calcula oportunidades de arbitraje y devuelve una lista de dicts con métricas:
    - symbol, buy_exchange, sell_exchange, buy_price, sell_price,
      profit_percent, success_prob
    """
    # 1. Obtener precios y guardar snapshot
    prices = await fetch_prices()
    await save_price_snapshot(prices)

    opportunities = []
    # 2. Recorrer permutaciones de exchanges y símbolos
    for buy_ex, sell_ex in permutations(EXCHANGES.keys(), 2):
        for symbol in SYMBOLS:
            buy_price = prices.get(buy_ex, {}).get(symbol)
            sell_price = prices.get(sell_ex, {}).get(symbol)
            if not isinstance(buy_price, (int, float)) or not isinstance(sell_price, (int, float)):
                continue
            
            profit = ((sell_price - buy_price) / buy_price) * 100
            if profit <= min_profit:
                continue
                     
            # try:
            #     success_prob = await predict_opportunity(buy_price, sell_price, profit)
            # except Exception as e:
            #     logging.warning(f"Predictor failed for {symbol}@{buy_ex}->{sell_ex}: {e}")
            #     success_prob = 0.0
            op = {
                "symbol": symbol,
                "buy_exchange": buy_ex,
                "sell_exchange": sell_ex,
                "buy_price": buy_price,
                "sell_price": sell_price,
                "profit_percent": round(profit, 3),
                "success_prob": round(0.8, 3),
            }
            opportunities.append(op)

    if opportunities:
        await save_opportunities(opportunities)
    return opportunities